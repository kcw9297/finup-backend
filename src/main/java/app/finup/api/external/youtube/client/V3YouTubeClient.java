package app.finup.api.external.youtube.client;
...
import app.finup.api.external.youtube.dto.YouTubeApiDtoMapper;
...
public class V3YouTubeClient implements YouTubeClient {
    ...
    @Override
    public YouTubeApiDto.Detail getVideo(String videoId) {
        ...
        return youTubeClient.get()
                ...
                .switchIfEmpty(Mono.error(new ProviderException(AppStatus.API_YOUTUBE_REQUEST_FAILED, "videoUrl")))

                // [5] 역직렬화 수행
                .map(json -> StrUtils.fromJson(json, YouTubeApiDto.VideosRp.class))

                // [6] 만약 비어있는 경우 예외 반환
                .filter(rp -> !rp.getItems().isEmpty())
                .switchIfEmpty(Mono.error(new ProviderException(AppStatus.API_YOUTUBE_URL_NOT_VALID, "videoUrl")))

                // [7] 최종적으로 외부에 제공할 DTO 변환
                .map(rp -> YouTubeApiDtoMapper.toDetail(rp, videoId))
                ...
                .onErrorMap(Exception.class, ex -> {
                    ...
                    return new ProviderException(AppStatus.API_YOUTUBE_REQUEST_FAILED, "videoUrl", ex);
                })
                .block();
    }

    @Override
    public List<YouTubeApiDto.Detail> getVideos(List<String> videoIds) {
        ...
        return youTubeClient.get()
                ...
                // [4] 빈 응답 처리 (응답 자체가 없는 경우)
                .switchIfEmpty(Mono.error(new ProviderException(AppStatus.API_YOUTUBE_REQUEST_FAILED)))

                // [5] 역직렬화 수행
                .map(json -> StrUtils.fromJson(json, YouTubeApiDto.VideosRp.class))

                // [6] 최종적으로 외부에 제공할 DTO 변환 (비어 있는 경우에 대한 예외는 처리하지 않음)
                .map(rp -> YouTubeApiDtoMapper.toDetails(rp, videoIds))

                // [7] 예외 통합 처리 (가장 마지막에 작성해야 함)
                .onErrorMap(Exception.class, ex -> {
                    LogUtils.showError(this.getClass(), "Youtube 영상 상세 API 요청 실패.\n원인 : %s", ex.getMessage());
                    if (ex instanceof ProviderException) return ex;
                    return new ProviderException(AppStatus.API_YOUTUBE_REQUEST_FAILED, ex);
                })
                .block();
    }
    ...
    public List<YouTubeApiDto.Row> searchVideo(String q) {
        ...
        return youTubeClient.get()
                ...
                .map(json -> StrUtils.fromJson(json, YouTubeApiDto.SearchRp.class))
                .map(YouTubeApiDtoMapper::toRows)
