    ...
    public static final String PROMPT_ANALYZE_CHART = """
        ...
        ### 분석 방법 [핵심]
            ...
            1. **구간별 흐름 파악**
               - 초기 (1~10번째): 과거 기반
               - 중기 (11~20번째): 추세 변화 여부
               - 최근 (21~30번째): 현재 상태 ← **가장 중요**
    
            2. **전환점 필수 언급**
               - 고점/저점 경신 후 방향 전환이 있었는지 반드시 확인
               - 있다면: "고점 경신 후 조정", "저점 이후 반등 시도" 등 명시
               - 없다면: "뚜렷한 전환점 없이 흐름 유지" 명시
    
            3. **최근 vs 과거 비교**
               - 최근 5~10개 캔들이 이전과 어떻게 다른지 비교
               - 예: "이전보다 변동폭 축소", "거래량 동반 상승"
    
        ### candleType별 관점
            - DAY: 최근 며칠간 단기 변동 중심
            - WEEK: 최근 몇 주간 중기 추세 변화
            - MONTH: 큰 흐름 속 방향 전환 여부
    
        ### 서술 원칙
            - 초보자 눈높이로 쉽게 설명
            - 단순 "상승세/하락세"가 아닌, **맥락과 변화** 설명
            - 거래량과 가격 변화의 **인과관계** 연결
            - 투자 조언, 매수/매도 권유 금지
            - 과거 분석 이력이 있으면 다른 관점 제시
    
        ### 출력 형식 (JSON만)
            {
              "summary": "전체 흐름 + 전환점 + 최근 상태를 종합한 요약",
              "trend": "전환점을 포함한 추세 설명 (예: 상승 후 조정, 이후 반등 시도)",
              "volatility": "최근 변동성이 이전 대비 어떻게 변했는지",
              "volumeAnalysis": "거래량과 가격 변화의 연관성 (예: 상승 시 거래량 동반 증가)"
            }
